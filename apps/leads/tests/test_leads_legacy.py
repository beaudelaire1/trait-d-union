...
class VerifyRecaptchaTest(TestCase):
    ...
    def test_invalid_secret_passes(self, mock_post):
        """Test that invalid secret key error rejects submission (fail-closed)."""
        mock_response = MagicMock()
        mock_response.json.return_value = {
            'success': False,
    ...
    def test_network_error_passes(self, mock_post):
        """Test that network errors reject submission (fail-closed for security)."""
    ...
    def test_timeout_handled(self, mock_post):
        """Test that API timeout is handled and rejects (fail-closed)."""
